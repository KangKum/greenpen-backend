...
app.use(express_1.default.static(path_1.default.join(__dirname, "../front-dist"))); // 또는 "dist"
let userCollection;
...
app.get("/worry/like/:commentId/:anonId", async (req, res) => {
    ...
    try {
        const comment = await worryLetterCommentsCollection.findOne({ _id: new mongodb_1.ObjectId(commentId) });
        ...
        if (comment.likes && comment.likes.includes(anonId)) {
            // 이미 좋아요를 눌렀으면 제거
            await worryLetterCommentsCollection.updateOne({ _id: new mongodb_1.ObjectId(commentId) }, { $pull: { likes: anonId } });
            if (writer !== anonId) {
            ...
            // 없으면 추가
            await worryLetterCommentsCollection.updateOne({ _id: new mongodb_1.ObjectId(commentId) }, { $addToSet: { likes: anonId } });
            if (writer !== anonId) {
                await userCollection.updateOne({ anonId: writer }, { $inc: { point: 2 } }, { upsert: true });
            }
...
app.get("/worry/dislike/:commentId/:anonId", async (req, res) => {
    ...
    try {
        ...
        if (comment.dislikes && comment.dislikes.includes(anonId)) {
            // 이미 싫어요를 눌렀으면 제거
            await worryLetterCommentsCollection.updateOne({ _id: new mongodb_1.ObjectId(commentId) }, { $pull: { dislikes: anonId } });
            if (writer !== anonId) {
            ...
            // 없으면 추가
            await worryLetterCommentsCollection.updateOne({ _id: new mongodb_1.ObjectId(commentId) }, { $addToSet: { dislikes: anonId } });
            if (writer !== anonId) {
                await userCollection.updateOne({ anonId: writer }, { $inc: { point: -1 } }, { upsert: true });
            }
...
app.get("/worry/:worryId/:anonId", async (req, res) => {
    ...
    const letter = await worryLetterCollection.findOne({ _id: new mongodb_1.ObjectId(worryId) });
    const writer = letter.anonId;
    if (letter.attention.includes(anonId)) {
        // 이미 공감한 상태이면 제거
        await worryLetterCollection.updateOne({ _id: new mongodb_1.ObjectId(worryId) }, { $pull: { attention: anonId } });
        if (writer !== anonId) {
            await userCollection.updateOne({ anonId: writer }, { $inc: { point: -3 } }, { upsert: true });
        }
        const updatedLetter = await worryLetterCollection.findOne({ _id: new mongodb_1.ObjectId(worryId) });
        res.status(200).json({ message: "공감 취소", attentionList: updatedLetter.attention });
    }
    else {
        // 공감하지 않은 상태이면 추가
        await worryLetterCollection.updateOne({ _id: new mongodb_1.ObjectId(worryId) }, { $addToSet: { attention: anonId } });
        if (writer !== anonId) {
            await userCollection.updateOne({ anonId: writer }, { $inc: { point: 3 } }, { upsert: true });
        }
        const updatedLetter = await worryLetterCollection.findOne({ _id: new mongodb_1.ObjectId(worryId) });
