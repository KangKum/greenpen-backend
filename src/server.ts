...
app.post("/worry/like/:commentId/:anonId", async (req, res) => {
  ...
  try {
    ...
    const comment = await worryLetterCommentsCollection.findOne(
      { _id: new ObjectId(commentId) },
      { projection: { likes: 1, anonId: 1 } } // 불필요한 데이터 제외
    );
    ...
    const writer = comment.anonId;
    const hasLiked = comment.likes?.includes(anonId);

    // 2️⃣ 좋아요 추가 / 제거 쿼리
    const updateQuery = hasLiked ? { $pull: { likes: anonId } } : { $addToSet: { likes: anonId } };

    // 3️⃣ 댓글 업데이트
    await worryLetterCommentsCollection.updateOne({ _id: new ObjectId(commentId) }, updateQuery);

    // 4️⃣ 작성자 포인트 업데이트
    if (writer !== anonId) {
      await userCollection.updateOne({ anonId: writer }, { $inc: { point: hasLiked ? -2 : 2 } });
    }

    // 5️⃣ 프론트가 즉시 UI 업데이트할 수 있도록 필요한 정보만 반환
    res.status(200).json({
      message: hasLiked ? "좋아요 취소" : "좋아요 성공",
      liked: !hasLiked,
      commentId,
    });
  } catch (error) {
...
app.post("/worry/dislike/:commentId/:anonId", async (req, res) => {
  ...
  try {
    // 1) 필요한 필드만 가져오기
    const comment = await worryLetterCommentsCollection.findOne({ _id: new ObjectId(commentId) }, { projection: { dislikes: 1, anonId: 1 } });
    ...
    const writer = comment.anonId;
    const hasDisliked = comment.dislikes?.includes(anonId);

    // 2) 싫어요 toggle 업데이트
    await worryLetterCommentsCollection.updateOne(
      { _id: new ObjectId(commentId) },
      hasDisliked ? { $pull: { dislikes: anonId } } : { $addToSet: { dislikes: anonId } }
    );

    // 3) 포인트 반영 (본인 댓글이면 적용 안 함)
    if (writer !== anonId) {
      await userCollection.updateOne({ anonId: writer }, { $inc: { point: hasDisliked ? 1 : -1 } });
    }

    // 4) 프론트에서 즉시 UI 반영할 수 있게 최소 정보만 반환
    res.status(200).json({
